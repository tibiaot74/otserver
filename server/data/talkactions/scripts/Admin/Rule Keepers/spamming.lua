...
resultname = mysqlQuery("SELECT `account_id`, `name`, `group_id` FROM `players` WHERE `players`.`name` = '".. param .."'", "account_id, name, id, group_id")

	if getNumberValue(resultname.id) > 0 and getNumberValue(resultname.group_id) < 6 then
...
UNBANDATE = ((os.time()) +7*24*60*60) --Never been banished before, smooth punishment
	...
	mysqlQuery("UPDATE `accounts` SET `unban_date` = ".. UNBANDATE ..", `ban_by` = \"".. getPlayerName(cid) .."\", `ban_type` = 1 WHERE `accounts`.`id` ='".. resultname.account_id .."';", "set")
